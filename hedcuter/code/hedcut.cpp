...
bool Hedcut::build(cv::Mat & input_image, cv::Mat & input_color_image, int n)
{
	//sample n points
	...
	//create disks
	create_disks(input_image, input_color_image, cvt);
...
void Hedcut::sample_initial_points(cv::Mat & img, int n, std::vector<cv::Point2d> & pts)
{
	...
	while (count < n)
	{
		...
		float gr = fabs(rng_gaussian.gaussian(-100));
		if ( value < 1-gr && visited.at<uchar>(r, c) ==0) //keep
		{
			count++;
...
void Hedcut::create_disks(cv::Mat & img, cv::Mat & color_img, CVT & cvt)
{
	disks.clear();
	...
	for (auto& cell : cvt.getCells())
	{
		...
		cv::Vec3b image_color = color_img.at<cv::Vec3b>(cell.site.x, cell.site.y);
		disk.color = (black_disk) ? cv::Scalar::all(0) : cv::Scalar(image_color);//cv::Scalar::all(avg_v);
